...
If you select \texttt{Interpolated point-to-point} in the GUI then the robot still receives two operational setpoints.
The final setpoint is \texttt{Md} and its corresponding joint position is in the vector \texttt{qf}. The previous setpoint is \texttt{M0} and its corresponding joint position is in the vector \texttt{q0}. Those joint positions come from the inverse geometry and should be correct at this point.\\
