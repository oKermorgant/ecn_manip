...
\begin{document}
  ...
  \begin{itemize}
    \item \okt{Robot::init_wMe()} to initialize the fixed transform between the wrist and end-effector
    \item \okt{Robot::fMw(q)} for the direct geometric model wrist-to-fixed frame
    \item \okt{Robot::inverseGeometry(M)} for...well, the inverse geometry
    \item \okt{Robot::fJw} for the kinematic model wrist-to-fixed frame
  \end{itemize}
  
  \subsection{Installing and loading the lab in Qt Creator}
  
  This project uses the ROS 2\footnote{Robot Operating System, http://www.ros.org} framework which imposes some particular steps to configure the environment. An actual ROS course will be held in the second semester, for now just configure as follows:

  \begin{enumerate}
    \item Open a terminal and go to the ROS 2 workspace:
    \begin{bashcodelarge}
mkdir -p ~/ros2/src
cd ~/ros2/src
    \end{bashcodelarge}
    \item Download this package:
\begin{bashcodelarge}
git clone https://github.com/oKermorgant/ecn_manip
\end{bashcodelarge}
\item Compile the package with ROS 2
 \begin{bashcodelarge}
cd ecn_manip
ros2ws
colbuild -t
\end{bashcodelarge}
\item Configure Qt Creator and load the project:
 \begin{bashcodelarge}
ideconf # or gqt on older VMs
\end{bashcodelarge}
    \item Load the \okt{ros2/src/ecn_manip/CMakeLists.txt} file through \okt{File...open project}
    \begin{itemize}
     \item You can also right-click open the file with "Qt Creator for ROS 2"
    \end{itemize}
  ...
  \begin{itemize}
    \item \okt{control.cpp}: main file where the control is done depending on the current robot mode
    \item \okt{robot_turret.cpp}: model of the RRP turret robot
    \item \okt{robot_kr16.cpp}: model of the industrial Kuka KR16 robot
    \item \okt{robot_ur10.cpp}: model of the Universal Robot UR-10
  \end{itemize}
  ...
  In the file \okt{control.cpp}, the \okt{if - else if} blocks correspond to the 6 exercises to be done for each robot. Note that if they work for the Turret robot, then
  ...
  \begin{itemize}
    \item \okt{Robot::init_wMe()} where you have to write the constant matrix $\wMe$ from the end-effector to the wrist frame
    \item \okt{Robot::fMw(q)}  where you have to write and return the transform $\M$ between the fixed and wrist frame
  \end{itemize}
  Once you have computed the \okt{M} matrix, the code calls \okt{robot->checkPose(M);} in order to print both your matrix and the true one. No need to tell that they should be equal.\\
  ...
  If you select \okt{Manual Operational Velocity} in the GUI then the sliders allow you to give a desired operational velocity (ie twist) for the end-effector.\\
  This velocity is ${}^e\v_e$ and is defined in the end-effector frame.\\
  The robot Jacobian has to be defined in the function \okt{Robot::fJw(q)} that should return the Jacobian of the wrist in the fixed frame.\\
  ...
  \begin{enumerate}
    ...
    Call \okt{M.getRotationMatrix()} to get this rotation.\\
    The \okt{ecn::putAt} function may be of good use.
    \item Map to the joint space: $\dot\q = \J^+.{}^f\mathbf{v}_e^*$\\
    Call \okt{robot->fJe(q).pseudoInverse()} to get the Jacobian pseudo-inverse
  \end{enumerate}
  ...
  If you select \okt{Direct point-to-point} in the GUI then the robot will receive two operational setpoints, at a given sampling time that can be changed with the \okt{Switching time} slider in the GUI. The current setpoint is in the matrix \okt{Md} and its corresponding joint position is in the vector \okt{qf}.
  
  Nothing has to be done in the main code, but of course the \okt{Robot::inverseGeometry(M)} function has to be filled in order to compute the actual inverse geometry solution. The robot will try to have all joints reach their desired value as soon as possible, which will lead to a discontinuous motion.\\
  ...
  When computing the inverse geometry, try to find a solution as near as possible to the passed \okt{q0}.\\
  ...
  A given potential solution should be stored using \okt{addCandidate(\{q1, q2, ..., qn\});}\\ At the end of the Inverse Geometry function, use \okt{return bestCandidate(q0);} that will pick the candidate that is the nearest from the current joint position and also within joint limits.
  
  \subsection*{Exercise 4: Interpolated point-to-point control}
  
  In this exercise the previous pose setpoint \okt{M0} and its corresponding joint position \okt{q0} should be used to interpolate the joint positions.\\
  
  Here the goal is to compute the current setpoint $\q_c = \q_0 + p(t)(\q_f-\q_0)$. The interpolation function $p(t)$ was defined during the lectures and should take into account the maximum joint velocity and acceleration that are stored in the \okt{vMax} and \okt{aMax} vectors. Basically this amounts to computing the minimum time \okt{tf} needed to reach $\q_f$ from $\q_0$. This computation should be done in the \okt{if(robot->newRef())} and then used to compute the current \okt{qCommand}.
  ...
  \okt{M0} and \okt{Md} and command the robot to reach sequentially all these poses. As they will be pretty close one from each other, the resulting motion
  will be close to a straight line.
  
  The \okt{robot->intermediaryPose(M0, Md, alpha)} function returns a pose between \okt{M0} and \okt{Md}, where \okt{alpha} is between 0 and 1 and should be computed from $t$, $t_0$ and $t_f$ = 1 s.
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item in the code, ${}^{f}\M_{e*}$ corresponds to \okt{Md} and ${}^{f}\M_e$ to \okt{M}
      \item In practice we use the $(\mathbf{t}, \theta\u)$ representation with \okt{p.buildFrom()}
    \end{itemize}
  ...
  \end{enumerate}
  $\lambda$ is a gain that can be changed from the GUI. Increasing it will lead to a faster convergence. It can be obtained through \okt{robot->lambda()}.
  ...
  \begin{itemize}
    \item \okt{vpMatrix}: a classical matrix of given dimensions, can be multiplied with other matrices and vectors if the dimensions are ok. The pseudo-inverse of matrix \okt{M}
    is available with \okt{M.pseudoInverse()}
    \item \okt{vpColVector}: a column vector of given dimension
    \item \okt{vpHomogeneousMatrix}: the classical $4\times 4$ 3D transform matrix
  \end{itemize}
  ...
  To write 1 at element (i,j) of matrix M, just write \okt{M[i][j] = 1;}\\
  Similarly, to write 1 at element (i) of vector q, just write \okt{q[i] = 1;}\\
  ...
  \newpage
   ...
   A section \okt{constants} should be added in order to get the numerical values:
  ...
  Then just run:
  \begin{bashcode}
   ros2 launch ecn_manip mdh_launch.py mdh:=<your file.yaml>
  \end{bashcode}
  ...
  As it is quite tedious to have to compute all matrices when a solution of MDH is investigated, a tool is provided to generate the C++ code. An example can be found in the file \okt{dh_example.yml}:
  \cppstyle
  \begin{lstlisting}
  notation: [alpha, a, theta, r]
  joint:
  ...
  4: [-pi/2, a4, q4+pi/2, 0]
  \end{lstlisting}
  The command to be run is: \okt{ros2 run ecn_manip dh_code.py <name of the file>}.\\
  ...
  Fixed transforms for initial and end-effector frames can also be written with keys \okt{f:} and \okt{e:}\\

  If passing \okt{--display} the script will display the direct geometry with classical math notations. It can be useful for the inverse geometry computation.\\
  For a 6-DOF robot, passing \okt{--wrist} will display useful information to benefit from a spherical wrist: math expressions for $\Pass{0}{\mathbf{t}}{6}(q_1,q_2,q_3)$ and  $\Pass{3}{\R}{6}(q_4,q_5,q_6)$, and code for $\Pass{0}{\R}{3}(q_1,q_2,q_3)$.
  ...
  \begin{itemize}
    \item A \okt{std::vector<double>} for equations of 1 unknown (types 2 \& 3).\\We can go through the valid solutions with:
    \cppstyle
    \begin{lstlisting}
    ...
    \end{lstlisting}
    \item A \okt{std::vector<JointSolution>} for equations of 2 unknowns (types 4 to 8).\\In this case, the valid solutions can be explored with:
    \cppstyle
    \begin{lstlisting}
...
Note that the $(x_x, \hdots, z_z)$ terms are not the same as when decomposing the full $\Pass{0}{\M}{n*}$ matrix.
  ...
  \begin{tabular}{|c|c|C{7cm}|C{2cm}|}\hline
    ...
    2 & $Xs_i+Yc_i = Z$ & \okt{solveType2(x, y, z)}&$q_i$\\\hline
    3 & $\begin{array}{l}X_1s_i+Y_1c_i = Z_1 \\ X_2s_i+Y_2c_i = Z_2\end{array}$&\okt{solveType3(x1, y1, z1,
      x2, y2, z2)} &$q_i$\\\hline
    4 & $\begin{array}{l}X_1q_js_i = Y_1 \\ X_2q_jc_i = Y_2\end{array}$& \okt{solveType4(x1, y1, x2, y2)}&$(q_i,q_j)$ \\\hline
    5 & $\begin{array}{l}X_1s_i = Y_1+Z_1q_j \\ X_2c_i = Y_2+Z_2q_j\end{array}$& \okt{solveType5(x1, y1, z1,
      x2, y2, z2)}&$(q_i,q_j)$\\\hline
    6 & $\begin{array}{l}Ws_j = Xc_i + Ys_i+Z_1 \\ Wc_j = Xs_i - Yc_i+Z_2\end{array}$& \okt{solveType6(x, y,
      z1, z2, w)}&$(q_i,q_j)$\\\hline
    7 & $\begin{array}{l}W_1c_j +W_2s_j = Xc_i + Ys_i+Z_1 \\ W_1s_j -W_2c_j = Xs_i - Yc_i+Z_2\end{array}$& \okt{solveType7(x, y, z1, z2, w1, w2)}&$(q_i,q_j)$\\\hline
    8 &$\begin{array}{l}Xc_i+Yc_{ij} =Z_1 \\ Xs_i+Ys_{ij} =Z_2\end{array}$ & \okt{solveType8(x, y, z1, z2)}&$(q_i,q_j)$\\\hline
  \end{tabular}
